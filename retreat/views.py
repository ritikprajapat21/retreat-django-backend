...
def fetchRetreats(request):
    ...
    print(search, page)
    retreats = Retreat.objects.all().filter(Q(title__icontains=filter) | Q(description__icontains=filter) | Q(title__icontains=search) | Q(description__icontains=search) | Q(condition__icontains=search) | Q(condition__icontains=filter) | Q(location__iexact=location)).values()
...
def book(request):
    ...
    try:
        ...
        booking = Booking(date=booking_date, payment_detail=payment_details, retreat=retreat, user=user)
